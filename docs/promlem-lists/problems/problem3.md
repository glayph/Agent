...
- **Fix:** Added environment variables for sandbox configuration (`MIKI_SANDBOX_FILESYSTEM`, `MIKI_SANDBOX_NETWORK`, `MIKI_SANDBOX_SECRETS`, `MIKI_SANDBOX_SHELL`) in `buildPluginEnvironment()` and passed sandbox configuration through plugin execution flow in `plugin-contract-runtime.ts:655-689, 708-734, 934-971`.
