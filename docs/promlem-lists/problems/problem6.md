...
- Evidence:
  ...
  - `packages/core/src/api/index.ts:229-236` derives the actual `workspaceDir` once from `MIKI_WORKSPACE_DIR` or the inferred repo root, then constructs `new AgentOrchestrator(workspaceDir)`.
