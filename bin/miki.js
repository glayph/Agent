...
async function gatewayHealthy() {
  ...
  try {
    ...
    return Boolean(body && body.status === "ok" && body.coreHealthy === true);
  } catch {
    return false;
  }
}
...
async function waitForReadySignal() {
  // Poll the real health endpoint instead of trusting console output timing.
  // "Gateway listening" can print in the child's inherited stdio before the
  // core backend has actually finished its own startup/health check, so
  // clients that fire requests as soon as they see that line can hit
  // connection-refused errors on slower machines.
  const deadline = Date.now() + 60_000;
  while (Date.now() < deadline) {
    if (shuttingDown || !child) return;
    if (await gatewayHealthy()) {
      console.log("Miki is ready — the gateway and core backend are both healthy.");
      return;
    }
  ...
  if (!shuttingDown && child) {
    console.warn(
      "Miki: gateway did not report healthy within 60s. It may still be starting; check the logs above.",
    );
  }
}
