...
$RestartDelayMs = [int](Get-EnvOrDefault "SUPERVISOR_RESTART_DELAY_MS" "5000")

if ($MaxRestarts -eq -1) {
    $MaxRestarts = [int](Get-EnvOrDefault "SUPERVISOR_MAX_RESTARTS" "0")
}

$WebhookUrl = Get-EnvOrDefault "SUPERVISOR_WEBHOOK_URL" ""
...
# Do not remove control files automatically: an operator may have created a stop
# sentinel while the previous supervisor was shutting down. Clear only the
# exhaustion marker after a deliberate new start.
