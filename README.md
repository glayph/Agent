...
For detailed Linux/Windows service setup, reboot/crash drills, delivery idempotency, model smoke tests, soak testing, and production notes, read [`docs/24-7-operations.md`](docs/24-7-operations.md) and [`SETUP.md`](SETUP.md).
...
| `npm run model:smoke -- --local` | Opt-in local llama.cpp/LFM transport smoke test |
| `npm run model:smoke -- --gemini` | Opt-in Gemini transport smoke test |
| `npm run soak -- --duration-minutes 60` | Record bounded health/resource soak observations |
