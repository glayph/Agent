...
- **Sessions are created implicitly.** There is no `POST /sessions` endpoint. A client generates its own `session_id` (e.g. `crypto.randomUUID()`) and sends it with the first message to `POST /api/chat` — the orchestrator creates the session record on first use. `GET /api/sessions` only lists sessions that already exist.
- **A model provider must be configured before chat will produce real replies.** A fresh install ships with a default model reference (Gemini) but no API key, so `status` is `unconfigured`. Until you add a valid key under Models/Credentials (or install a local model via `npm run model:install`), `/api/chat` responds with `status: "success"` and an explanatory error message in `response` (e.g. "The gemini credential was missing or rejected...") rather than a crash — this is expected, not a bug, but it means chat is not usable out of the box.
- **Wait for readiness before calling the API.** After `npm start`, watch the console for `Core backend is healthy` (not just `Gateway listening`) before sending requests — on slower machines the backend can take several seconds longer to come up, and requests sent before that line appears will fail with a connection error.
