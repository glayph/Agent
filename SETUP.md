...
## 14. 24/7 deployment and target-host acceptance

The detailed Linux systemd and Windows Task Scheduler installers, crash/reboot drill instructions, idempotent delivery boundary, LFM/Gemini smoke tests, and bounded soak harness are maintained in [`docs/24-7-operations.md`](docs/24-7-operations.md). Use that runbook after the normal build and first dashboard smoke test. A local build or generated service file is not evidence of a real OS reboot, credentialed channel delivery, external MCP reconnect, or multi-hour production soak.
