...
class TemporalKnowledgeGraph {
  ...
  addEntityRelation(sourceId, targetId, relationType, metadata = {}) {
    ...
    for (const oldEdge of existingActive) {
      ...
      if (similarity >= this.REPEAT_SIMILARITY_THRESHOLD) {
        ...
        this.db.prepare('UPDATE entity_edges SET weight = ? WHERE id = ?').run(newWeight, oldEdge.id);
        reinforced = true;
      ...
      if (similarity >= this.CONTRADICTION_SIMILARITY_THRESHOLD) {
        ...
        // newer, more-trusted version.
        const decayedWeight = Math.max(0, (oldEdge.weight || 1.0) * this.CONTRADICTION_DECAY_FACTOR);
        this.db.prepare('UPDATE entity_edges SET weight = ? WHERE id = ?').run(decayedWeight, oldEdge.id);
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, sourceId, targetId, relationType, weight, metadata.validFrom || now, metadata.validUntil || null, JSON.stringify(metadata), now);
  ...
  getContextWindow(queryStr, maxEvents = 20) {
    ...
    // ── Inter-day neural connections ────────────────────────────────────────
    // Surface related past-day summaries via the daily_summary_edges graph so
    // the LLM has long-term continuity beyond the 24 h recent-events window.
    // We start from today's date key and traverse up to 3 connected days,
    // ordered by edge weight (cosine similarity) descending. This is a
    // lightweight read — no LLM call, just SQLite lookups.
    try {
      const todayKey = this._getDateKey(new Date());
      const connected = this.getConnectedDailySummaries(todayKey, 3);
      if (connected.length > 0) {
        parts.push('=== Related Past Days (Neural Connections) ===');
        for (const conn of connected) {
          ...
          parts.push(`[${conn.dateKey}] (relevance: ${weight}) ${snippet}`);
        }
        parts.push('');
      }
    } catch {
      // getConnectedDailySummaries is best-effort — a missing table or any
      // other DB issue must never break the primary context window output.
    }
