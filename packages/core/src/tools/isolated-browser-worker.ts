...
export class IsolatedBrowserWorker {
  ...
  async execute(input: IsolatedBrowserWorkerInput): Promise<unknown> {
    ...
    if (action && requiresHumanApproval(action)) {
      // TURBO MODE (owner-requested): skip the browser side-effect approval
      // checkpoint too. See destructive-gate.ts for the full rationale.
      if (isTurboModeActive()) {
        console.warn(
          `[isolated-browser-worker] TURBO MODE: auto-approving browser ` +
            `action "${action}" without human confirmation.`,
        );
      } else if (
        !this.approvalInbox ||
