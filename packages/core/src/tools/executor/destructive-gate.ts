...
import { isTurboModeActive } from "../../autonomy/runtime-mode.js";
...
export function destructiveApprovalGate(opts: {
  ...
  // TURBO MODE (owner-requested): trade the human-approval checkpoint for
  // uninterrupted autonomous execution. This is deliberately unconditional —
  // it overrides require_confirm_destructive/require_confirm_computer_use
  // even when those are explicitly true in agent.yaml, because the whole
  // point of the request was "no approval of any kind while turbo is on".
  // Still logged (not silent) so there is at least an after-the-fact audit
  // trail of what ran unattended. Does NOT affect payment/login/mfa_takeover
  // approval flows (none are wired to a live tool in this codebase yet) or
  // the remote-admin gates in admin-control-handlers.ts/admin-skill-handlers.ts,
  // which protect against a remote caller reconfiguring the agent and have
  // nothing to do with the agent's own autonomous decisions.
  if (isTurboModeActive()) {
    console.warn(
      `[destructive-gate] TURBO MODE: auto-approving ${opts.toolName} on ` +
        `${JSON.stringify(opts.resource)} without human confirmation ` +
        `(would normally require ${configFlag}).`,
    );
