...
export class ShellExecutor {
  ...
  public async runShell(
    ...
    // BUG-01 FIX: Hardcoded blocklist for unconditionally dangerous commands.
    // These patterns fire before any user-configurable deny list so they cannot
    // be accidentally disabled. They cover the most common catastrophic forms of
    // `rm -rf /`, `mkfs`, `dd if=/dev/zero`, fork-bomb, and full-disk wipe.
    const ALWAYS_DENIED_PATTERNS: ReadonlyArray<RegExp> = [
      // rm -rf / (with or without --no-preserve-root, various spacing)
      /rm\s+(-[^\s]*f[^\s]*|-[^\s]*r[^\s]*)\s+.*\/\s*$/i,
      /rm\s+.*--no-preserve-root/i,
      // mkfs on a bare block device (mkfs.ext4 /dev/sda, mkfs /dev/xvda …)
      /mkfs(\.\w+)?\s+\/dev\//i,
      // dd writing zeros/random to a block device
      /dd\s+.*if=\/dev\/(zero|random|urandom).*of=\/dev\//i,
      // fork bomb  :(){ :|:& };:
      /:\(\)\s*\{.*:\|:.*\}/,
      // shred on block device
      /shred\s+.*\/dev\//i,
      // wipefs on any device
      /wipefs\s+.*\/dev\//i,
    ];
    const trimmedCmd = command.trim();
    for (const re of ALWAYS_DENIED_PATTERNS) {
      if (re.test(trimmedCmd)) {
        return {
          stdout: "",
          stderr: "",
          exitCode: -1,
          error:
            `shell_execute blocked: the command matches a built-in safety ` +
            `pattern and cannot be run regardless of config. ` +
            `If this was intentional, run it manually outside of Miki.`,
        };
      }
    }
