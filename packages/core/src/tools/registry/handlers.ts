...
export async function handleShellExecute(
  ...
  // BUG-08 FIX: gate destructive shell commands behind ApprovalInbox when
  // tools.require_confirm_destructive is enabled in agent.yaml. Previously
  // this handler called executor.runShell() unconditionally regardless of
  // approvalInbox — the only protection was the hardcoded catastrophic
  // blocklist inside ShellExecutor itself, which does not cover ordinary
  // (but still irreversible) destructive commands like `rm -rf <subdir>`.
  ...
  if (isDestructiveShellCommand(cmd)) {
    const { gate, response } = destructiveApprovalGate({
      ...
      resource: `shell:${cmd.slice(0, 200)}`,
      args,
    });
...
export async function handleFileDelete(
  ...
  // BUG-08 FIX: file_delete is always destructive — gate it unconditionally
  // (dry-run requests are exempt since they make no actual change).
  ...
  const output = this.fileOps.deleteFile(path, isDryRun);
  if (approvalHandle) {
    consumeDestructiveApproval(this.approvalInbox, approvalHandle);
  }
