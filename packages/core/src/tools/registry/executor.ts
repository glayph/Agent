...
export class ToolRegistry {
  ...
  // BUG-02 FIX: Levenshtein distance for fuzzy "did you mean?" suggestions.
  ...
  private _suggestToolName(attempted: string): string | undefined {
    const allNames = Array.from(this.handlers.keys());
    let best: string | undefined;
    let bestDist = Infinity;
    for (const name of allNames) {
      const dist = ToolRegistry._editDistance(attempted, name);
      if (dist < bestDist) {
        bestDist = dist;
        best = name;
      }
    }
    // Only suggest if edit-distance is within 40% of the attempted name's
    // length — too far apart means nothing useful to suggest.
    const threshold = Math.max(3, Math.floor(attempted.length * 0.4));
    return bestDist <= threshold ? best : undefined;
  }

  // BUG-03 FIX: Validate required schema parameters before calling handler.
  ...
  private async runHandler(
    ...
    // BUG-03 FIX: validate required args before dispatching to the handler.
    const validationError = this._validateArgs(name, args);
    if (validationError) {
