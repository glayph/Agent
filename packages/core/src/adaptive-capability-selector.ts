...
function explicitToolNames(userMessage: string): Set<string> {
  ...
  // "delete" by default). Without this, file_delete could never be offered
  // to the model on a simple, no-verification delete request.
  if (intent.kind === "file_delete") return new Set(["file_delete"]);
  return new Set();
}
