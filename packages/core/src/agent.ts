...
interface AgentConfigShape {
  ...
  heartbeat?: {
    ...
    /** Legacy on/off switch, superseded by the `autonomy` block below but
     * still read for backward compatibility (never its action-count field). */
    auto_actions?: { enabled?: boolean; max_actions_per_cycle?: number };
  };
  /** Persistent autonomous goal system config (packages/core/src/autonomy). */
...
export class AgentOrchestrator {
  ...
  public autonomy: AutonomyController | null = null;
  ...
  constructor(paths: RuntimePaths | string) {
    ...
    // ── Persistent autonomous goal system ────────────────────────────────
    // Reuses this.taskScheduler (durable queue + retries + concurrency) as
    // the executor and this.concurrentManager for resource awareness —
    // see packages/core/src/autonomy/README for the design writeup.
    try {
      const autonomyRawConfig = asAgentConfig(this.config).autonomy;
      const legacyAutoActions = asAgentConfig(this.config).heartbeat
        ?.auto_actions;
      const autonomyDb = new Database(
        path.join(runtimePaths.dataDir, "autonomy.db"),
      );
      this.autonomy = createAutonomyController({
        db: autonomyDb,
        schedule: (sessionId, message, cronExpression, runAt, options) =>
        ...
        isProviderHealthy: () =>
          !(
            this.selfImprovement as unknown as {
              _circuitBreaker?: { tripped: boolean };
            }
          )._circuitBreaker?.tripped,
        hints: () =>
          (autonomyRawConfig?.hints as Record<string, unknown> | undefined) ??
          {},
        config: parseAutonomyConfig(autonomyRawConfig, legacyAutoActions),
      });
    } catch (autonomyErr) {
      // Mirrors the memory-init failure handling above: autonomy is an
      // enhancement, never a reason the agent itself fails to start.
      console.error(
        "[Agent] Autonomy controller init failed (continuing without autonomy):",
        (autonomyErr as Error).message,
      );
      this.autonomy = null;
    }
  ...
  async reloadConfig(): Promise<void> {
    ...
    if (this.autonomy) {
      ...
      const legacyAutoActions = asAgentConfig(this.config).heartbeat
        ?.auto_actions;
      const reloaded = parseAutonomyConfig(
        autonomyRawConfig,
        legacyAutoActions,
      );
      this.autonomy.setEnabled(reloaded.enabled);
      this.autonomy.setMode(reloaded.mode);
    }
  ...
  stopBackgroundTasks(): Promise<void> {
    ...
    if (this.autonomy) tasks.push(this.autonomy.shutdown());
  ...
  private async _callLlmApi(
    ...
    if (toolsSchema && toolsSchema.length > 0) {
      ...
      // Small local checkpoints often support the tools schema but do not
      // reliably emit a structured call when a prompt can be answered
      // directly. Requiring a tool on every local turn can leave the agent
      // waiting for a call that never arrives; auto keeps tool access while
  ...
  async *runAgentLoop(
    ...
    // Let plain chat control autonomy ("miki use turbo mode", "pause
    // autonomy") without needing the Web UI (spec section 16/17: the goal
    // is backend control surfaces, and the simplest one is the chat the
    // user is already using). Deliberately checked before any LLM call —
    // mirrors the deterministic-intent short-circuit a little further down.
    if (this.autonomy) {
      const commandReply = this.autonomy.handleChatCommand(userMessage);
      if (commandReply !== null) {
        ...
        this._logMemoryInteraction(sessionId, userMessage, commandReply);
