...
describe("event runtime contracts", () => {
  ...
  it("persists delivery receipts, deduplicates sends and dead-letters failures", async () => {
    ...
    expect(queue.replay(first.id, "delivery-2")?.id).toBe(
      queue.getByIdempotencyKey("delivery-2")?.id,
    );
    fs.rmSync(dir, { recursive: true, force: true });
  });

  it("requires reconciliation before replaying an unknown provider outcome", async () => {
    const dir = fs.mkdtempSync(
      path.join(os.tmpdir(), "miki-delivery-unknown-"),
    );
    const queue = new DeliveryQueue(path.join(dir, "deliveries.json"));
    ...
    await queue.dispatch(async () => ({
      ...
      error: "request timed out after provider accepted the connection",
    }));
    expect(queue.get(first.id)?.status).toBe("unknown_outcome");
    expect(queue.replay(first.id, "unknown-replay")).toBeNull();
    expect(
      queue.reconcile(first.id, {
        status: "sent",
        providerMessageId: "provider-1",
      })?.status,
    ).toBe("sent");
    expect(queue.getByIdempotencyKey("unknown-1")?.providerMessageId).toBe(
      "provider-1",
    );
    fs.rmSync(dir, { recursive: true, force: true });
  });
