...
export class ContextualToolPruner {
  ...
  // Guaranteed-recovery tools: always kept available so the agent has a safe
  // read-only path when heuristics are uncertain. These do not receive an
  // inflated relevance score (see semanticToolScore) — being "always present"
  // and being "semantically preferred" are different things, and conflating
  // them previously caused web_search to outrank the correct tool (or the
  // absence of any tool) for self-contained requests like summarization.
  private readonly FALLBACK_TOOLS = ["memory_search", "web_search", "ask_user"];
  // Of the fallback tools, only these are safe to force into the result even
  // when their score is below the relevance threshold: both are read-only
  // and inexpensive, so surfacing them for an ambiguous turn cannot itself
  // produce a wrong or misleading answer. web_search is deliberately excluded
  // from this guaranteed set: unlike a no-op read, an unnecessary web search
  // can fail outright and produce a visible "search failed" response in place
  // of the answer the user actually asked for. web_search still appears
  // normally whenever it earns its score semantically or via route/skill
  // preference.
  private readonly GUARANTEED_FALLBACK_TOOLS = ["memory_search", "ask_user"];
  ...
  /**
   * Priority order used to break ties when a query matches more than one
   * context's keywords (e.g. "research and write a report" matches both
   * research and creation). Order mirrors the previous hard-coded checks.
   */
  private readonly CONTEXT_PRIORITY = [
    "programming",
    "data_analysis",
    "vision",
    "learning",
  ...
  private inferContext(query: string): string {
    ...
    // Context inference now reads from the same CONTEXT_KEYWORDS table used
    // for scoring, so a keyword that is meant to signal a context (like
    // "summarize" for learning) actually routes the query there instead of
    // only ever affecting the score for a context it can never trigger.
    for (const context of this.CONTEXT_PRIORITY) {
  ...
  getPrunedToolset<T extends Tool>(
    ...
    // Keep read-only/recovery fallbacks available, but respect the turn
    // budget. Only the guaranteed subset is force-included below threshold;
    // web_search must still earn its place in topK like any other tool.
    const pruned = Array.from(
      new Map(
        [
          ...topK,
          ...allTools.filter((t) =>
            this.GUARANTEED_FALLBACK_TOOLS.includes(this.getToolName(t)),
          ),
  ...
  private semanticToolScore(tool: Tool, context: string): number {
    ...
    const text = [
      ...
      .toLowerCase();
    const keywords = this.CONTEXT_KEYWORDS[context] || [];
    // All tools start from the same baseline. A fallback tool like web_search
    // still needs to match the inferred context (or a route/skill
    // preference, scored separately in scoreRoutePreference) to rank above
    // an unrelated tool — it no longer receives an unconditional head start
    // that let it outscore the correct tool for self-contained requests.
    let score = this.GUARANTEED_FALLBACK_TOOLS.includes(name) ? 0.8 : 0.2;
