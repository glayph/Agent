...
describe("SQLite durable task queue", () => {
  it("persists the complete task envelope and blocks duplicate delivery", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "miki-sqlite-"));
    dirs.push(dir);
    ...
    const task = first.enqueue("canonical-session", "full atomic message", {
      ...
      channel: "telegram",
      idempotencyKey: "telegram:msg-9",
      artifactRefs: ["artifact://a"],
    })!;
    const duplicate = first.enqueue("other-session", "must not execute", {
      idempotencyKey: "telegram:msg-9",
    });
    ...
    const final = new TaskQueue({ maxSize: 10, databasePath }).getTask(
      task.id,
    )!;
