...
// BUG-05 FIX: Keys for providers that exist in the vault allow-list but have
// no actual provider plugin wired up. Saving them should emit a clear warning
// so operators know the key is stored but will have no effect at runtime.
const UNWIRED_PROVIDER_KEYS: ReadonlySet<string> = new Set([
  "OPENAI_API_KEY",
  "OPENROUTER_API_KEY",
...
export function createLauncherCompatRouter({
  ...
  const handleLogin = (req: Request, res: Response) => {
    ...
    // Surface whether the default chat model actually has a usable
    // credential/runtime, so the UI can prompt the user to finish setup
    // right away instead of them only finding out after a chat request
  ...
  // NOTE: There is no `POST /sessions` endpoint. Chat sessions are created
  // implicitly: the client generates a session_id (e.g. crypto.randomUUID())
  // and sends it with the first message to `POST /api/chat`. The orchestrator
  // creates the session record on first use of that id. This route only
  // lists sessions that already exist.
