...
export class DeliveryQueue {
  ...
  enqueue(
    ...
    if (existing && existing.status !== "dead_letter") return existing;
  ...
  /**
   * Find a delivery by the caller-supplied idempotency key. This is the
   * reconciliation primitive for at-least-once workers: after a timeout or
   * crash, callers must inspect this record before issuing another side effect.
   */
  getByIdempotencyKey(idempotencyKey: string): DeliveryReceipt | null {
  ...
  /**
   * Resolve an unknown provider outcome only after an operator or provider
   * lookup confirms whether the external side effect happened. Never infer
   * success from a network timeout.
   */
  reconcile(
    receiptId: string,
    result: Extract<DeliveryAttemptResult, { status: "sent" | "failed" }>,
  ): DeliveryReceipt | null {
    const receipt = this.receipts.get(receiptId);
    if (!receipt || receipt.status !== "unknown_outcome") return null;
    return this.settle(receiptId, result);
  }
