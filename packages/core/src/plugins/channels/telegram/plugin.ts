...
export const telegramChannelPlugin: BuiltinChannelPlugin = {
  manifest: channelManifest({
    ...
    runtime_note:
      "Telegram adapter supports durable long polling or secret-verified webhooks, canonical sessions, allowlists, rate limits, duplicate protection, threaded replies, attachments, and delivery retry.",
  ...
  createRouter: (orchestrator: AgentOrchestrator) => {
    ...
    router.post("/", async (req, res) => {
      const configured = resolveTelegramRuntimeConfig(
        orchestrator.config,
      ).webhookSecret;
      if (
        configured &&
