...
export interface IOrchestrator {
  ...
  /**
   * Optional hook into the autonomous goal system (packages/core/src/autonomy).
   * Kept as a single narrow method here so HeartbeatEngine's own contract
   * doesn't need to know anything about goals, objectives, or scheduling —
   * all of that lives in the autonomy module itself.
   */
  autonomy?: {
    tick: (
      idleMins: number,
...
export class HeartbeatEngine {
  ...
  private async _pulse(): Promise<void> {
    ...
    // The autonomy module owns its own idle-threshold/mode gating
    // internally (see AutonomyController.tick), so it is invoked on every
    // pulse rather than only when idleMins > 5 like self-improvement above;
    // Turbo mode in particular wants a much shorter idle threshold than the
    // fixed 5 minutes used for self-improvement cycles.
    if (this.orchestrator.autonomy) {
      try {
        await this.orchestrator.autonomy.tick(
          idleMins,
          disk as Record<string, unknown>,
        );
      } catch (e: unknown) {
        console.warn(`[Heartbeat] Autonomy tick failed: ${getErrorMessage(e)}`);
      }
    }
