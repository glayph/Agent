...
describe("AgentOrchestrator workflow acceleration", () => {
  ...
  it("finalizes a bounded tool-only loop with a safe summary", async () => {
    ...
    expect(events).toContainEqual(
      expect.objectContaining({
        type: "stream_chunk",
        // BUG-07 FIX: the fallback message was rewritten to be a clear,
        // user-facing summary (in Bengali, per project convention) instead of
        // the previous opaque "Completed tool steps: X." text. Assert on the
        // tool name still being named in the summary rather than the exact
        // old English phrasing.
