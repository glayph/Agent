declare module "http-proxy-middleware" {
  ...
  interface Options extends HttpProxyOptions {
    ...
    pathRewrite?: { [regexp: string]: string } | ((path: string, req: IncomingMessage) => string);
    router?: { [host: string]: string } | ((req: IncomingMessage) => string);
    logLevel?: "debug" | "info" | "warn" | "error" | "silent";
    logProvider?: (provider: LogProvider) => LogProvider;
    on?: {
      proxyReq?: (proxyReq: any, req: IncomingMessage, res: ServerResponse) => void;
  ...
  export function createProxyMiddleware(options?: string | Options): RequestHandler;
  export { Options };
}

declare module "http-proxy-middleware/dist/context-matcher" {
  export function match(context: any, uri: string, req: any): boolean;
}
